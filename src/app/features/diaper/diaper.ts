...
export class DiaperPage {
  ...
  readonly storageError = this.diaperService.storageError;
  readonly selectedType = signal<DiaperType | null>(null);
  readonly savedType = signal<DiaperType | null>(null);
  ...
  register(): void {
    ...
    const diaper = this.diaperService.register(type);

    this.savedType.set(diaper.type);
    this.savedAt.set(diaper.recordedAt);
    this.selectedType.set(null);
  }

  label(type: DiaperType): string {
    return this.diaperService.label(type);
  }
}