...
export class BabyContextService {
  ...
  async ensureLoaded():
    ...
    /*
     * Esconde imediatamente qualquer
     * bebê pertencente à sessão anterior.
     */
    this.state.set({
      uid,
      baby: null,
    ...
    try {
      ...
      if (
        ...
        this.state.set({
          ...
          loading: false,
          error:
            'Não foi possível carregar os dados do bebê.',
        });
      }
  ...
  private async loadForUser(
    ...
    if (
      ...
      throw new Error(
        'O vínculo com o bebê está inconsistente.',
      );
    }
